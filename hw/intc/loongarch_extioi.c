                          ...
                          uint64_t val, unsigned size)
{
    ...
    case EXTIOI_IPMAP_START ... EXTIOI_IPMAP_END - 1:
	/*
         * ipmap is set at the beginning of intr driver,
         * need not update upper irq level
         */
        index = offset - EXTIOI_IPMAP_START;
    ...
    case EXTIOI_ENABLE_START ... EXTIOI_ENABLE_END - 1:
        ...
        /* unmask irq */
        val = val & old_data;
        val &= s->sw_pending[index];
        s->sw_pending[index] &= ~val;
        irq = ctz32(val);
        while (irq != 32) {
            /*
             ...
             */
            extioi_update_irq(s, irq + index * 32, 1);
            val &= ~(1 << irq);
            irq = ctz32(val);
        }
